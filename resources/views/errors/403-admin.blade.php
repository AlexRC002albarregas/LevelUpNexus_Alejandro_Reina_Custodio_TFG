<x-layouts.app :title="'Solo para administradores - LevelUp Nexus'">
	<div class="max-w-3xl mx-auto text-center py-20">
		<div class="w-24 h-24 mx-auto rounded-3xl bg-red-600/20 border border-red-500/40 flex items-center justify-center text-red-400 text-4xl mb-6 glow">
			<i class="fas fa-shield-alt"></i>
		</div>
		<p class="uppercase tracking-[0.5em] text-sm text-red-300 mb-2">Acceso restringido</p>
		<h1 class="text-4xl md:text-5xl font-black bg-gradient-to-r from-pink-400 to-purple-500 bg-clip-text text-transparent mb-4">
			El panel de control es exclusivo para administradores
		</h1>
		<p class="text-purple-200/80 max-w-2xl mx-auto mb-10">
			Parece que estás intentando entrar al centro de mando sin los permisos necesarios.
			Si necesitas realizar algún cambio, contacta con el equipo administrador para que te habiliten acceso.
		</p>
		<div class="flex flex-col sm:flex-row items-center justify-center gap-4">
			<a href="{{ route('landing') }}" class="w-full sm:w-auto px-6 py-3 rounded-xl border border-purple-500/40 text-purple-100 hover:bg-purple-500/10 transition">
				<i class="fas fa-home"></i> Ir al inicio
			</a>
			<a href="{{ route('posts.index') }}" class="w-full sm:w-auto px-6 py-3 rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 font-semibold text-white transition glow">
				<i class="fas fa-newspaper"></i> Volver al feed
			</a>
		</div>
		<p class="text-xs text-purple-300/70 mt-8">
			¿Crees que esto es un error? Escribe a <a href="mailto:hello@levelupnexus.com" class="underline hover:text-white">hello@levelupnexus.com</a>
		</p>
	</div>
</x-layouts.app>

